...
"$MMSEQS_BIN" prefilter "$QUERY_DB" "$TARGET_DB" "$CPU_DB" --exact-kmer-matching 1 -k 7 --spaced-kmer-pattern "10111111"  --diag-score 0 --threads $(nproc) -v 3 2>&1 | tee "$OUT_DIR/kmer_cpu.log"
...
"$MMSEQS_BIN" prefilter "$QUERY_DB" "$TARGET_DB" "$DPU_DB" --exact-kmer-matching 1 -k 7 --spaced-kmer-pattern "10111111"  --dpu 1 -v 3 2>&1 | tee "$OUT_DIR/kmer_dpu.log"
if [ ${PIPESTATUS[0]} -ne 0 ]; then
